...
function getModelRouter() { if (!_modelRouter) _modelRouter = new ModelRouter(process.env.OPENAI_API_KEY || ''); return _modelRouter; }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { model, intent } = await getModelRouter().smartRoute(message);
    ...
    const completion = await getModelRouter().chatWithFallback(
      {
        messages: [
    ...
    const cost = getModelRouter().calculateCost(model, inputTokens, outputTokens);
    ...
    if (assistantMessage.tool_calls) {
      ...
      for (const toolCall of assistantMessage.tool_calls) {
        ...
        const secondCompletion = await getModelRouter().chatWithFallback(
          {
            messages: [
              { role: 'system', content: systemPrompt },
              ...messages,
