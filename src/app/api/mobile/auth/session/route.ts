...
import { getCustomerFresh } from '@/lib/odoo/customers';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let user: MobileUser = {
      ...
      email: payload.email,
      name: payload.name,
      odooPartnerId: payload.odooPartnerId,
      priceListId: payload.priceListId,
      currencyCode: payload.currencyCode,
    };

    // Get fresh data from API if requested
    const refreshData = request.nextUrl.searchParams.get('refresh') === 'true';
    if (refreshData && payload.odooPartnerId) {
      const customer = await getCustomerFresh(payload.odooPartnerId);
      if (customer) {
        user = {
          ...user,
