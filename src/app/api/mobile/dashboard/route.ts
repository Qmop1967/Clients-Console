...
import { getCustomerBalance } from '@/lib/odoo/customers';
import { getCustomerOrders } from '@/lib/odoo/orders';
import { getCustomerInvoices } from '@/lib/odoo/invoices';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [balanceData, ordersData, invoicesData] = await Promise.all([
      getCustomerBalance(user.odooPartnerId),
      getCustomerOrders(user.odooPartnerId, 1, 5), // Last 5 orders
      getCustomerInvoices(user.odooPartnerId, 1, 5), // Last 5 invoices
    ]);
