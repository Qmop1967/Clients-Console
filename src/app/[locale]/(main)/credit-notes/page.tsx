...
import { getCustomerCreditNotes, getAvailableCreditBalance } from "@/lib/odoo/credit-notes";
...
async function fetchCreditNotes(
  ...
  try {
    return await getCustomerCreditNotes(odooPartnerId, page, 25);
  } catch (error) {
...
export default async function CreditNotesPage({
  ...
  const [creditNotesResponse, totalAvailableCredit] = await Promise.all([
    fetchCreditNotes(session.user.odooPartnerId, currentPage),
    session.user.odooPartnerId
      ? getAvailableCreditBalance(session.user.odooPartnerId)
      : 0,
  ]);
