...
import { getCustomerOrders, getOrderSummaryStats, type OrderSummaryStats } from "@/lib/odoo/orders";
...
export default async function OrdersPage({
  ...
  let ordersResponse: PaginatedResponse<SalesOrder> = { data: [], page_context: EMPTY_PAGE_CONTEXT };
  let summaryStats: OrderSummaryStats = EMPTY_SUMMARY_STATS;

  const results = await Promise.allSettled([
    getCustomerOrders(odooPartnerId, currentPage, 25),
    getOrderSummaryStats(odooPartnerId),
  ]);
